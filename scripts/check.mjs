...
assert.ok(
  shortPnl(entry, 100, AMOUNT) > 0,
  "падение курса при шорте = прибыль",
);
assert.equal(
  shortPnl(entry, entry, AMOUNT),
  0,
  "закрытие по цене открытия = ноль",
);
...
for (const s of scenarios) {
  console.log(
    `   ${s.pair.padEnd(9)} → ${s.loss} ${s.pair.endsWith("JPY") ? "¥" : s.pair.startsWith("RUB") ? "$" : s.pair.endsWith("RUB") ? "₽" : "$"}`,
  );
}
